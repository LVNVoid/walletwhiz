...
export const StatsCard: React.FC<StatsCardProps> = ({
  ...
  return (
    <Card
      className={cn(
        "group relative overflow-hidden transition-all duration-300 hover:shadow-lg hover:shadow-zinc-200/20 dark:hover:shadow-zinc-800/20 hover:-translate-y-0.5",
        styles.card,
        className
      )}
    >
      ...
      <CardContent className="relative p-6">
        <div className="flex items-start justify-between">
          ...
          {Icon && (
            <div
              className={cn(
                "flex items-center justify-center w-10 h-10 rounded-lg transition-all duration-300 group-hover:scale-105",
                styles.icon
              )}
            >
        ...
        {/* Subtle bottom accent - only for highlighted cards */}
        {isHighlighted && (
